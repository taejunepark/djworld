	...
	$(document).ready(function() {
        ...
        $("#write_btn").click(function(){
       	 if(validation()){
       		 ...
       		 // form에 추가
       		 $("form").append(input)
        		 $("form").attr('action', '${pageContext.request.contextPath }/minihome/${owner.id }/board_edit/${board.no}').submit()
       	 }
...
<div class="highlight">
	<form action="#" method="post">
		<div id="titleArea">
			<input type="text" name="title" value="${board.title }"placeholder="제목을 입력하세요" required>
		</div>
		
		<div id="detailArea">
			<textarea name="detail" id="detail" rows="10" cols="100">${board.detail }</textarea>
		</div>
	</form>
	<div id="optionArea">
		<button id="write_btn">완료</button>
		<button onclick="location.href='${pageContext.request.contextPath}/minihome/${userId }/board_detail/${board.no }'">취소</button>
	</div>
</div>
