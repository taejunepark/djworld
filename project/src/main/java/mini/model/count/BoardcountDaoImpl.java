...
public class BoardcountDaoImpl implements BoardcountDao {
	...
	public int diaryCount(String id) {
		String sql = "select count(*) from diary where owner=?";
		return jdbcTemplate.queryForObject(sql, Integer.class, id);
	}
	
	@Override
	public int diaryTodayCount(String id) {
		String sql = "select count(*) from diary where to_char(reg, 'yyyymmdd' ) = to_char( sysdate, 'yyyymmdd') and owner = ?";
		return jdbcTemplate.queryForObject(sql, Integer.class, id);
	}
	...
	public int photoTodayCount(String id) {
		String sql = "select count(*) from photo where to_char(reg, 'yyyymmdd' ) = to_char( sysdate, 'yyyymmdd') and owner = ?";
		return jdbcTemplate.queryForObject(sql, Integer.class, id);
	}
